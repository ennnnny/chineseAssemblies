...
namespace EasyEzreal
{
    class EazyEzreal : Champion
    {
        ...
        {
            Menu.AddSubMenu(new Menu("閫ｆ嫑", "Combo"));
            ...
            Menu.AddSubMenu(new Menu("楱锋摼", "Harass"));
            ...
            Menu.AddSubMenu(new Menu("鑷嫊", "Auto"));
            Menu.SubMenu("Auto").AddItem(new MenuItem("Auto_q", "浣跨敤Q").SetValue(true));
            Menu.SubMenu("Auto").AddItem(new MenuItem("Auto_w", "浣跨敤W").SetValue(false));
            Menu.SubMenu("Auto").AddItem(new MenuItem("Auto_r", "浣跨敤R").SetValue(true));
            Menu.SubMenu("Auto").AddItem(new MenuItem("Auto_minrange", "R鏈€灏忕瘎鍦?").SetValue(new Slider(1050, 0, 1500)));
            Menu.SubMenu("Auto").AddItem(new MenuItem("Auto_maxrange", "R鏈€澶х瘎鍦?").SetValue(new Slider(3000, 1500, 5000)));
            Menu.SubMenu("Auto").AddItem(new MenuItem("Auto_manaE", "鐐篍淇濇寔钘嶉噺").SetValue(true));

            Menu.AddSubMenu(new Menu("椤ず", "Drawing"));
            Menu.SubMenu("Drawing").AddItem(new MenuItem("Drawing_q", "Q绡勫湇").SetValue(new Circle(true, Color.FromArgb(100, 0, 255, 0))));
            Menu.SubMenu("Drawing").AddItem(new MenuItem("Drawing_w", "W绡勫湇").SetValue(new Circle(true, Color.FromArgb(100, 0, 255, 0))));
            Menu.SubMenu("Drawing").AddItem(new MenuItem("Drawing_rdamage", "R鍌峰鎸囩ず").SetValue(true));
        }
