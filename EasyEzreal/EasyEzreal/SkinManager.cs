...
class SkinManager
{
    ...
    public void AddToMenu(ref Menu menu)
    {
        ...
        if (Skins.Count > 0)
        {
            Menu.AddSubMenu(new Menu("鏀瑰彉鐨偆", "Skin Changer"));
            Menu.SubMenu("Skin Changer").AddItem(new MenuItem("Skin_" + ObjectManager.Player.ChampionName + "_enabled", "闁嬪暉鎻涜啔").SetValue(false));
            Menu.SubMenu("Skin Changer").AddItem(new MenuItem("Skin_" + ObjectManager.Player.ChampionName + "_select", "鐨啔").SetValue(new StringList(Skins.ToArray())));
